...
from core.models import Achat_Stock_Carburant_HT, Service
...
try:
    ...
    nouvelle_dotation = Achat_Stock_Carburant_HT(
        service=service,
        fournisseur='TotalEnergies Cu00f4te d\'Ivoire (Carburant)',
        ...
        numero_de_voucher=f'DV-{datetime.now().strftime("%Y%m%d")}-001'
    )
    ...
    nouvelle_dotation_essence = Achat_Stock_Carburant_HT(
        ...
        numero_de_voucher=f'DV-{datetime.now().strftime("%Y%m%d")}-002'
    )
